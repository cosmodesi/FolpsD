...
chain_name = f'chains/MCMC-fs_klim_0-0.02-{k_max}_2-0.02-{k_max}-{tracers_str}-GCcomb_schoneberg2024-bbn_planck2018-ns10_{prior_basis}-prior_{model}_{pt_model}v2_new'
if not set_emulator:
    chain_name += '_noemu'
#print(chain_name)
...
for tracer in tracers:
    ...
    if pt_model == "lpt_velocileptors":
        ...
        theory = FOLPSv2TracerPowerSpectrumMultipoles(template=template, prior_basis="physical") #Add the prior_basis='physical' argument to use physically motivated priors
...
if set_emulator:
    ...
    for i in range(len(theories)):
        emulator_filename = f'Emulator_test/emu_-{tracers_str}_{str(tracers[i])}.npy'
