...
def create_observable(tracer, comp):
    ...
    if comp == "ps":
        return TracerPowerSpectrumMultipolesObservable(
            data=data[:len(kr_pk)],
            covariance=cov_arr[:len(kr_pk), :len(kr_pk)],
            theory=theories[tracer]["ps"],
            klim=k_thy, ells=[0, 2], kin=[kr_pk], wmatrix=m_bin[k_points_pk]
        )
    elif comp == "bs":
        ...
        return TracerPowerSpectrumMultipolesObservable(
            ...
            klim=k_thy, ells=[0, 2], kin=[kr_b0, kr_b2],
            wmatrix=m_bin[np.concatenate([k_points_b0, k_points_b2])]
        )
