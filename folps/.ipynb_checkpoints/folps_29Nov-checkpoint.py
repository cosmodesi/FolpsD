...
class BackendManager:
    ...
    def setup_backend(self, preferred_backend='numpy'):
        ...
        if preferred_backend == 'jax':
            try:
                import jax
                gpu_devices = [d for d in jax.devices() if d.device_kind in ["Gpu", "Metal", "GPU", "gpu"]]
                
                if gpu_devices:
                    print(f"✅ GPU detected: {gpu_devices[0].device_kind}. Using JAX with GPU.")
                else:
                    print("⚠️ No GPU found. Using JAX with CPU.")
                    print("   If you have an NVIDIA GPU, install: pip install -U 'jax[cuda12]'")

                from jax import numpy as np
                from tools_jax import interp, simpson, legendre, extrapolate, extrapolate_pklin, get_pknow, get_linear_ir, get_linear_ir_ini, qpar_qperp, get_pknow_jax, interp_at_kmin, roots_legendre
                self.modules = {
                    "np": np,
                    "interp": interp,
                    "simpson": simpson,
...
# `get_pknow_jax`, `interp_at_kmin`, and `roots_legendre` exist only when JAX backend is selected. 
...
if backend == 'jax':
    ...
    roots_legendre_jax = backend_manager.get_module("roots_legendre")
else:
    get_pknow_jax = None
    interp_at_kmin = None
    roots_legendre_jax = None
...
class BispectrumCalculator:
    ...
    def tablesGL_f(self, precision=[4, 5, 5]):
        ...
        Nphi, Nx, Nmu = map(int, precision)
        Pi = np.pi

        # Use backend-aware roots_legendre function
        if backend == 'jax':
            # Use JAX-native implementation
            phi_roots, phi_weights = roots_legendre_jax(Nphi)
            x_roots, x_weights = roots_legendre_jax(Nx)
            mu_roots, mu_weights = roots_legendre_jax(Nmu)
        else:
    ...
    def sigmas(self, kT,pklT):
        ...
        Sigma2_ = 1/(6 * np.pi**2)*simpson(pklT_*(1 - spherical_jn_backend(0, kT_/k_BAO) 
                                                + 2*spherical_jn_backend(2, kT_/k_BAO)), x=kT_)
        deltaSigma2_ = 1/(2 * np.pi**2)*simpson(pklT*spherical_jn_backend(2, kT/k_BAO), x=kT)
    ...
    def Bisp_Sugiyama(self, f, bpars, k_pkl_pklnw, z_pk,
                      ...
                      m_bin=None, k_thy=None, do_binning=False,do_interp=False,kout=None):
