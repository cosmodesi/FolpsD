...
name=f"c_FolpsD_{tracer}_z{z_evaluation:.3f}_Pkmax-{k_max:.3f}_B000kmax-{k_max_b0:.3f}_B202kmax-{k_max_b2:.3f}_bsfree_folpspy"
...
def Folps(h, omega_cdm, omega_b, logA_s, b1, b2, bs2, c1,c2,Pshot,Bshot, a_vir, a_vir_bk, k_ev):
    ...
    qpar, qper = qpar_qperp(Omega_fid=Omfid, Omega_m=Omega_m, z_pk=z_pk)

    nonlinear = NonLinearPowerSpectrumCalculator(mmatrices=mmatrices,
                                                 kernels='fk',
                                                 **kwargs)

    table, table_now = nonlinear.calculate_loop_table(k=ps['k'], pklin=ps['pk'],cosmo=None, **kwargs)
    
    multipoles = RSDMultipolesPowerSpectrumCalculator(model='FOLPSD')
    ##Pℓ,const
    P0_const, P2_const, P4_const  = get_rsd_pkell_marg_const(kobs=k_ev, qpar=qpar, qper=qper, pars=pars,
                                       table=table, table_now=table_now,
                                       bias_scheme='folps', damping='lor', model = 'FOLPSD'
                                       )
    
    ##Pℓ,i=∂Pℓ/∂α_i
    Pkl0_i, Pkl2_i, Pkl4_i = get_rsd_pkell_marg_derivatives(kobs=k_ev, qpar=qpar, qper=qper, pars=pars,
                                       table=table, table_now=table_now,
                                       bias_scheme='folps', damping='lor', model = 'FOLPSD'
                                       )
    
    #Binning for Pℓ,const
    Pkl0_const_mbin = m_bin @ P0_const 
    Pkl2_const_mbin = m_bin @ P2_const 
    ...
    #np.array([LoopCorrections[0][0], LoopCorrections[0][1], LoopCorrections[1][1]])

    
    bpars = [b1, b2, bs2, c1,c2,Bshot,Pshot, a_vir_bk]
    ...
    if isB000:
        ...
        B000__, B202__ = bispectrum.Bisp_Sugiyama(f=f0, bpars=bpars,
                                                 k_pkl_pklnw=k_pkl_pklnw, z_pk=z_pk, k1k2pairs=k_ev_bk,
                                                 qpar=qpar, qperp=qper)#, 
                                               #, Omfid=-1,precision=precision)
        B000_ = interp(k_thy,k_ev_bk[:,0],B000__)
        B202_ = interp(k_thy,k_ev_bk[:,0],B202__)
...
print(numberofpk0points)
print(numberofbk0points)
print(numberofbk2points)
...
a_vir_min=0; a_vir_max=10;
a_vir_bk_min=0; a_vir_bk_max=15;
...
def log_prior(theta):
    ...
    Omb=omega_b/h**2            # Baccoemu has boundaries 
    if (Omb>0.07 or Omb<0.03):
        lp = -np.inf
...
def log_likelihood(theta, Hexa = False):
    ...
    if (h_min < h < h_max and
        ...
        L0 = compute_L0(Pl_const=md_const, Pl_data=data, invCov=cov_inv_arr)
        L1i = compute_L1i(Pl_i=md_i, Pl_const=md_const, 
                                Pl_data=data, invCov=cov_inv_arr)
        L2ij = compute_L2ij(Pl_i=md_i, invCov=cov_inv_arr)
        ...
        term1 = startProduct(L1i, L1i, invL2ij)
