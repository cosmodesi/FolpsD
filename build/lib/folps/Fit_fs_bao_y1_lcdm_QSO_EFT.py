...
eft_tracers = ['QSO']
eft_kmax= [0.201]
k_max_array = [0.301,0.301,0.301,0.301,0.301,0.201]
...
chain_name = f'/global/cfs/cdirs/desicollab/users/prakharb/folpsD_paper_chains/y1_chains/base_QSO_EFT/MCMC-fs+bao_klim_0-0.02-{k_max}_2-0.02-{k_max}-{tracers_str}-GCcomb_schoneberg2024-bbn_planck2018-ns10_{prior_basis}-prior_{model}_{pt_model}_Ano_eft_18_ACPT'
...
for tracer in tracers:
    ...
    if pt_model == "lpt_velocileptors":
        ...
        theory = FOLPSv2TracerPowerSpectrumMultipoles(template=template, prior_basis=prior_basis,A_full=False, b3_coev=True,sigma8_fid=sigma8_fid) #Add the prior_basis='physical' argument to use physically motivated priors
...
def create_observable(data_fn, theory, tracer_index,k_max):
    ...
    print(indices,data.flatx.shape)
...
observables = [create_observable(params['data_fn'], theories, i,k_max_array[i]) 
