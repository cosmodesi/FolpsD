...
chain_name = (
    f"{base_dir}/cxxx_mocks_chains/c002/base/{tracers_str}"
    ...
    f"_{'b3_coev' if b3_coev else 'b3_marg'}"
    f"{f'_damping_{damping}' if damping != 'lor' else ''}_TCM_priors_cdm"
)
    ...
    if pt_model == "rept_velocileptors":
        ...
        if pt_model == 'TNS':
            ps_theory = FOLPSv2TracerPowerSpectrumMultipoles(template=template,
               ...
               ps_theory = theory = FOLPSv2TracerPowerSpectrumMultipoles(template=template, prior_basis=prior_basis,A_full=A_full_status, b3_coev=b3_coev, sigma8_fid=fiducial.sigma8_m,h_fid=fiducial.h,use_TCM_priors=True)
...
def load_data_vector_and_covariance(tracer='LRG2',z_ev=0.8,k_max=0.301,k_max_b0=None,k_max_b2=None,P4=False):
    ...
    # k_data_2nd,Pk_0_2nd,Pk_2_2nd,Pk_4_2nd,B000_2nd,B202_2nd =  ExtractDataAbacusSummit_additionalcosmologies('c000')
    # Read covariance
    k_data_2nd,Pk_0_2nd,Pk_2_2nd,Pk_4_2nd,B000_2nd,B202_2nd =  ExtractDataAbacusSummit_additionalcosmologies('c002')
...
if set_emulator:
    for tracer in tracers:
        for comp in ["ps"]:  # handle PS and BS separately
            ...
            emulator_filename = f'{base_dir}/Emulators_c002/Emulator_{comp}/{comp}_emu_{tracer}_z{z}_{kr_max}_Afull_{A_full_status}_new2.npy'
