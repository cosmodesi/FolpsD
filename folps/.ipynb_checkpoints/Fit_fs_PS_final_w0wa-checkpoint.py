...
chain_name = (
    f"{base_dir}/Abacus2gen_chains/base_w_wa/{tracers_str}"
    ...
    f"{f'_damping_{damping}' if damping != 'lor' else ''}_noemu"
)
...
if set_emulator:
    for tracer in tracers:
        for comp in ["ps"]:  # handle PS and BS separately
            ...
            emulator_filename = f'{base_dir}/Emulators_w0wa_mocks/Emulator_{comp}/{comp}_emu_{tracer}_z{z}_{kr_max}_Afull_{A_full_status}.npy'
