...
def true_divide(h0, h1, out=None, where=None):
    ...
    # Build a denominator that never hits zero where the quotient is evaluated.
    den = np.where(where_arr, h1, np.ones_like(h1))
    den_nonzero = den != 0
    den_safe = np.where(den_nonzero, den, np.ones_like(den))
    quot = h0 / den_safe

    valid_where = np.logical_and(where_arr, den_nonzero)
    return np.where(valid_where, quot, out)
...
def qpar_qperp(Omega_fid, Omega_m, z_pk, cosmo=None):
    ...
    if Omega_fid<0 return qpar, qperp = 1,1   
    """
    if Omega_fid <0:
        return 1, 1
