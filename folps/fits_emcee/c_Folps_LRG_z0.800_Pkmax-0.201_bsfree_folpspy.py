...
name=f"c_Folps_{tracer}_z{z_evaluation:.3f}_Pkmax-{k_max:.3f}_bsfree_folpspy"
...
def Folps(h, omega_cdm, omega_b, logA_s, b1, b2, bs2, c1,c2,Pshot,Bshot, a_vir, a_vir_bk, k_ev):
    ...
    sigma8 = ps['sigma8']
    Omega_m = ps['Omega_m']
    Omega_mnu = ps['Omega_ncdm']

    kwargs = {'z': z_pk,
              'h': h,
              'Omega_m': Omega_m, #'f0':0.6900011469771554,
    ...
    P0_const, P2_const, P4_const  = get_rsd_pkell_marg_const(kobs=k_ev, qpar=qpar, qper=qper, pars=pars,
                                       table=table, table_now=table_now,
                                       bias_scheme='folps', damping=None, model='EFT'
                                       )
...
out=Folps(0.7, 0.12, 0.02, 3.0, 2, 0.1, -0.2, 0.2, 0.1,100,1000, 0,0, k_thy)
...
def log_prior(theta):
    ...
    h, omega_cdm, omega_b, logA_s, b1, b2, bs2 = theta
