...
pt_model = "folpsD"  #Choose b/w folpsD and EFT
...
chain_name = (
    f"{base_dir}/cxxx_mocks_chains/c003/base/Pk_Bk_bugfixed/{tracers_str}"
    ...
    f"{f'_damping_{damping}' if damping != 'lor' else ''}_ns_planck_centre"
)
...
#Update cosmo priors
for param in ['n_s', 'h','omega_cdm', 'omega_b', 'logA', 'tau_reio','N_eff']:
    ...
    if param == 'N_eff':
        cosmo.params[param].update(prior = {'dist':'uniform','limits': [0.01, 10.0]})
...
def load_data_vector_and_covariance(tracer='LRG2',z_ev=0.8,k_max=0.301,k_max_b0=None,k_max_b2=None,P4=False):
    ...
    # Read covariance
    k_data_2nd,Pk_0_2nd,Pk_2_2nd,Pk_4_2nd,B000_2nd,B202_2nd =  ExtractDataAbacusSummit_additionalcosmologies('c003')
...
if set_emulator:
    for tracer in tracers:
        for comp in ["ps",'bs']:  # handle PS and BS separately
            ...
            if comp=='ps':            
                emulator_filename = f'{base_dir}/Emulators_c003/Emulator_{comp}/{comp}_emu_{tracer}_z{z}_{kr_max}_Afull_{A_full_status}.npy'
            else: 
                kr_b2_max_label= kr_b2_max if kr_b2_max>0.08 else 0.08
                emulator_filename = f'{base_dir}/Emulators_c003/Emulator_{comp}_bugfixed/{comp}_emu_{tracer}_z{z}_{kr_max}_{kr_b0_max}_{kr_b2_max_label}_Afull_{A_full_status}.npy'
