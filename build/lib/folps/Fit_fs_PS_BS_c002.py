...
chain_name = (
    f"{base_dir}/cxxx_mocks_chains/c002/base/Pk_Bk_bugfixed/{tracers_str}"
    ...
    f"{f'_damping_{damping}' if damping != 'lor' else ''}_w_wa_cdm"
)
...
if set_emulator:
    for tracer in tracers:
        for comp in ["ps",'bs']:  # handle PS and BS separately
            ...
            if comp=='ps':            
                emulator_filename = f'{base_dir}/Emulators_c002_new/Emulator_{comp}/{comp}_emu_{tracer}_z{z}_{kr_max}_Afull_{A_full_status}.npy'
            else: 
                kr_b2_max_label= kr_b2_max if kr_b2_max>0.08 else 0.08
                emulator_filename = f'{base_dir}/Emulators_c002_new/Emulator_{comp}_bugfixed/{comp}_emu_{tracer}_z{z}_{kr_max}_{kr_b0_max}_{kr_b2_max_label}_Afull_{A_full_status}.npy'
