...
from desilike.observables import ObservableCovariance
from helper_cov import construct_bao_cross_power
...
chain_name = f'chains/fs_bao_EFT_newcov_phys_inf'
if profiler:
...
def create_observable(data_fn, theory, tracer_index,tracer=None):
    #Load and process covariance
    covariance = ObservableCovariance.load(data_fn)
    data_orig = covariance.observables('power')
    
    data,_,covariance, wmatrix_fn = construct_bao_cross_power(tracer,k_max)
    covariance = covariance.select(xlim=(0.02, k_max), projs=[0, 2],observables='power',select_projs=True)
    indices = np.where((data.flatx > 0.02) & (data.flatx < k_max))[0]  #new-line
    wshotnoise = np.concatenate([
    np.ones(int(len(indices)/2)),
    np.zeros(int(len(indices)/2))
])

    
    # print(data.attrs['wshotnoise'],data.attrs.keys())
    ...
    return TracerPowerSpectrumMultipolesObservable(
        data=data,
        covariance=covariance._value,
        klim={ell: [0.02, k_max, 0.005] for ell in [0, 2]},
        theory=theories[tracer_index],
        kin=data.attrs['kin'],
        wmatrix=wmatrix_fn,
        ellsin=data_orig.attrs['ellsin'],
        wshotnoise=wshotnoise
        
    )

#Create observables for each tracer
observables = [create_observable(params['data_fn'], theories, i,tracers[i]) 
...
def create_bao_observable(data_fn, tracer_index,tracer=None):
    ...
    observable_orig = BAOCompressionObservable(data=data.view(), covariance=covariance, cosmo=cosmo, quantities=data.projs, fiducial=fiducial, z=data.attrs['zeff'])

    
    _,data,covariance,_= construct_bao_cross_power(tracer,k_max)
    print(data.projs)
...
bao_observables = [create_bao_observable(params['data_fn'], i,tracers[i]) 
                for i, params in tracer_params.items()]
...
for i in range(len(observables)):
    ...
    tracer=tracers[i]
    observable_power, observable_bao, cov,_ = construct_bao_cross_power(tracer,k_max)
   
    likely = ObservablesGaussianLikelihood(observables = [observables[i],bao_observables[i]], covariance = cov)
    # likely = ObservablesGaussianLikelihood(observables = [observable_power, observable_bao], covariance = cov)
    likely()
...
if profiler:
    ...
    if sampler == 'cobaya':
        if restart_chain is False:
            sampler = MCMCSampler(sum(Likelihoods), save_fn = chain_name,covariance=cov_chain)
