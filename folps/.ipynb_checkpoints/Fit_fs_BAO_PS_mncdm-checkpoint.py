...
from desilike.theories.galaxy_clustering.full_shape import FOLPSv2TracerPowerSpectrumMultipoles, FOLPSAXTracerPowerSpectrumMultipoles
...
chain_name = f'/global/cfs/cdirs/desicollab/users/prakharb/folpsD_paper_chains/y1_chains/base/MCMC-fs+bao_klim_0-0.02-{k_max}_2-0.02-{k_max}-{tracers_str}-GCcomb_schoneberg2024-bbn_planck2018-ns10_{prior_basis}-prior_{model}_{pt_model}_Ano_tight_eft_FOLPSAX'
if not set_emulator:
    chain_name += '_noemu'
#print(chain_name)
...
for tracer in tracers:
    ...
    if pt_model == "lpt_velocileptors":
        ...
        theory = FOLPSv2TracerPowerSpectrumMultipoles(template=template, prior_basis=prior_basis) #Add the prior_basis='physical' argument to use physically motivated priors
...
if set_emulator:
    ...
    for i in range(len(theories)):
        emulator_filename = f'/global/cfs/cdirs/desicollab/users/prakharb/folpsD_paper_chains/Emulator_y1_mncdm/emu_-{tracers_str}_{str(tracers[i])}.npy'
...
if set_emulator:
    for i in range(len(bao_observables)):
        emulator_filename = f'/global/cfs/cdirs/desicollab/users/prakharb/folpsD_paper_chains/Emulator_y1_bao_mncdm/emu-bao_klim_0-0.02-{k_max}_2-0.02-{k_max}-{tracers_str}-GCcomb_schoneberg2024-bbn_planck2018-ns10_{prior_basis}-prior_{model}_{pt_model}_{str(tracers[i])}.npy'
