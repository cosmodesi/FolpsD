...
chain_name = (
    f"{base_dir}/Abacus2gen_chains/base_mncdm/Pk_Bk_bugfixed/{tracers_str}"
...
def make_params(prior_basis, width_EFT, width_SN0, width_SN2,pt_model='folpsD',b3_coev=True):
    ...
    if prior_basis == 'physical':
        ...
        if pt_model=='EFT':
            ...
            params['X_FoG_b'] = {'prior': {'dist':'uniform','limits': [0, 30]}}
...
if set_emulator:
    for tracer in tracers:
        for comp in ["ps",'bs']:  # handle PS and BS separately
            ...
            kr_max_label= kr_max if kr_max>0.301 else 0.301
            if comp=='ps':            
                emulator_filename = f'{base_dir}/Emulators_mncdm_mocks/Emulator_{comp}/{comp}_emu_{tracer}_z{z}_{kr_max}_Afull_{A_full_status}.npy'
            else: 
                kr_b2_max_label= kr_b2_max if kr_b2_max>0.08 else 0.08
                emulator_filename = f'{base_dir}/Emulators_mncdm_mocks/Emulator_{comp}_bugfixed/{comp}_emu_{tracer}_z{z}_{kr_max}_{kr_b0_max}_{kr_b2_max_label}_Afull_{A_full_status}.npy'
